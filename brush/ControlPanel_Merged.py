...
def runTrial(di_task, do_task, taskParameters, settings, trialIndex, totalTrials):
    ...
    if trialIndex == 0:
        ...
        print('first trial 2P trigger pulse')
    elif 0 < trialIndex < (totalTrials - 1):
        do_out[4, 5:pulseWidth] = True
        start2 = pulseWidth + gapWidth
        do_out[4, start2:start2 + pulseWidth] = True
        print('middle trial 2P trigger pulses')
    elif trialIndex == (totalTrials - 1):
        do_out[4, 5:pulseWidth] = True
        start2 = pulseWidth + gapWidth
        do_out[4, start2:start2 + pulseWidth] = True
        endPulseStart = numSamples - pulseWidth
        if endPulseStart < 0:
            endPulseStart = 0
        do_out[4, endPulseStart:endPulseStart + pulseWidth] = True
...
def runOperantTrial(di_task, ao_task, do_task, taskParameters, settings, trialIndex, totalTrials):
    # Timing calculations.
    numSamples = int(taskParameters['Fs'] * taskParameters['trialDuration'])
    stimTime_samples = int(taskParameters['stimTime'] * taskParameters['Fs'])
    stimDuration_samples = int(taskParameters['stimDuration'] * taskParameters['Fs'])
    ...
    if taskParameters.get('laserOutput', False):
        ...
        if taskParameters.get('laserOutput', False) and do_out.shape[0] > 5:
            for start_end in zip(laserStart_samples, laserEnd_samples):
                do_out[5, start_end[0]:start_end[1]] = True
...
def runTask(di_task, ao_task, do_task, taskParameters, settings):
    ...
    for trial in range(taskParameters['numTrials']):
        ...
        if taskParameters.get('conditioningMode', "Operant") == "Operant":
            di_data[trial], ao_data[trial], do_data[trial], result, laserLoc, pretrial_licks[trial] = runOperantTrial(di_task, ao_task, do_task, taskParameters, settings, trial, taskParameters['numTrials'])
...
def the_gui():
    ...
    left_column = [
        ...
        [sg.Text('Save Path', size=(textWidth,1)), sg.Input(os.path.normpath(r'Z:\DIRECTION\TEMP'), size=(20,1), key='-SavePath-'),
